...
def _get_predictor(config_path, model_path):
    ...
    logger.info(f"Merged config files from {config_path}")
...
def model_fn(model_dir):
    ...
    try:
        config_path = os.path.join(MODEL_EXTRACT_LOC, 'faster_rcnn_X_101_32x8d_FPN_3x.yaml')
...
def input_fn(request_body, request_content_type):
    ...
    try:
        if "application/x-npy" in request_content_type:
            input_object = decoder.decode(request_body, "application/x-npy")
