...
class AWSDetector:
    ...
    def __init__(self):
        ...
        # Defining the keypoint predictor. Object detector on AWS
        self.pred_keypt = DefaultPredictor(self.cfg_key)

    def img_to_aws(self, image_array):
        # First convert image array to base64 encoding
        ...
        predictions = json_to_d2(response.text, 'cuda')
        return predictions
    ...
    def video_detect(self, uploaded_video):
        ...
        def video_output(video):
            for n_frame in range(lift_start_frame, lift_end_frame):
                ...
                outputs_obj = self.img_to_aws(frame)
                # find top 2 largest boxes, these should be the plates closest to the camera
